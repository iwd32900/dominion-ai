...
class BasicPolicyGradientStrategy(Strategy):
    ...
    def __init__(self):
        ...
        obs_dim = 3 + n_acts # depends on impl. of self.state_idx()
    ...
    def state_idx(self, game, player):
        ...
        state = [t/20, s/13, p/3]
        invalid_act = [not b.can_move(game, player) for b in self.buys]
        return torch.as_tensor(state + invalid_act, dtype=torch.float)
    def start_game(self):
        self.last_s = None
        self.last_logp = None
